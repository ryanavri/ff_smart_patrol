  ...
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        actionButton("process_csv", "Process CSV Data", icon = icon("play")),
        width = 3
      ),
      mainPanel(
  ...
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        p("Configure parameters for the CPUE plot."),
  ...
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        actionButton("run_model", "Run Occupancy Model", icon = icon("play")),
        width = 3
      ),
      mainPanel(
        h4("Model Summary"),
...
server <- function(input, output, session) {
  ...
  model_results <- eventReactive(input$run_model, {
    ...
    withProgress(message = 'Running Occupancy Model...', value = 0, {
      # Step 1: Prepare data
      ...
      incProgress(0.1, detail = 'Setting model parameters...')
      chains <- 4
      ...
      incProgress(0.6, detail = 'Running model...')
      ...
      incProgress(0.1, detail = 'Predicting occupancy...')
      psi_pred <- predict(model_basic, submodel = "state")
      
      # Calculate naive occupancy
      incProgress(0.1, detail = 'Calculating naive occupancy...')
